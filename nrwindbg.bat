...
@echo ==== WINDBG_INIT_SCRIPT=%WINDBG_INIT_SCRIPT%
...
@for /f "delims=" %%i in ('dir/s/b *.pdb') do @if "%%~dpi" NEQ "%PREV%" @set PREV="%%~dpi"&(echo %%~dpi>> %PDBPATH_INIT_SCRIPT%)
@popd
@echo ^$^$^>^<%PDBPATH_INIT_SCRIPT%>> %WINDBG_INIT_SCRIPT%
:: load location of src files
@echo ^!homedir %DBGHELP_HOMEDIR%>> %WINDBG_INIT_SCRIPT%
@set SRCPATH_INIT_SCRIPT=%WINDBG_INIT_SCRIPT%_SRCpath
@echo .srcpath srv*> %SRCPATH_INIT_SCRIPT%
@pushd %rust_src_for_rs%
@for /f "delims=" %%i in ('dir/s/b/ad src') do @if "%%~dpi" NEQ "%PREV%" @set PREV="%%~dpi"&(echo %%~dpi>> %SRCPATH_INIT_SCRIPT%)
@popd
